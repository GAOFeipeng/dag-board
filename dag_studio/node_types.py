...
NODE_TYPES = [
    ...
    NodeTypeDefinition(
        id="data_import",
        label="Data Import",
        description="Load local CSV, NPY, or NPZ data uploaded through the import API.",
        inputs=[],
        outputs=["data", "graph"],
        output_ports=[
        ...
        inline_fields=["import_id", "standardize"],
        fields=[
            NodeField(name="import_id", label="Import ID", kind="string", default="", placeholder="upload first"),
            NodeField(name="x_key", label="X Key", kind="string", default="X"),
            NodeField(name="b_key", label="B Key", kind="string", default="B_true"),
            NodeField(name="w_key", label="W Key", kind="string", default="W_true"),
            NodeField(name="has_header", label="CSV Header", kind="boolean", default=True),
    ...
    NodeTypeDefinition(
        ...
        fields=[
            ...
            NodeField(name="seed", label="Seed", kind="integer", default=None, placeholder="empty"),
            NodeField(name="timeout_sec", label="Timeout Sec", kind="number", default=None, placeholder="empty"),
    ...
    NodeTypeDefinition(
        ...
        input_ports=[
            NodePort(id="data", label="Data", kind="data", required=True),
            NodePort(id="graph", label="Graph", kind="graph", required=False, min_count=0),
        ],
        ...
        inline_fields=["algorithms", "threshold", "timeout_sec"],
        fields=[
            NodeField(name="algorithms", label="Algorithms", kind="json", default=["PC", "GES"]),
            NodeField(name="param_grid", label="Param Grid", kind="json", default={}),
            NodeField(name="seeds", label="Seeds", kind="json", default=[None]),
            NodeField(name="metrics", label="Metrics", kind="json", default=["shd", "f1", "precision", "recall", "aupr"]),
    ...
    NodeTypeDefinition(
        id="graph_editor",
        label="Graph Editor",
        description="Apply edge edits to an input graph and output a validated DAG.",
        inputs=["graph"],
        outputs=["graph"],
        input_ports=[NodePort(id="graph", label="Graph", kind="graph", required=True)],
        output_ports=[NodePort(id="graph", label="Graph", kind="graph", required=False)],
        ...
        inline_fields=[],
        fields=[
            NodeField(name="edits", label="Edits", kind="json", default=[], placeholder='[{"op":"set_edge","source":"X1","target":"X2","weight":1.0}]'),
        ],
    ),
    NodeTypeDefinition(
        id="graph_view",
    ...
    NodeTypeDefinition(
        ...
        inputs=["data", "graph", "evaluation", "evaluation_summary", "graph_view"],
        outputs=["report"],
        input_ports=[
            NodePort(id="data", label="Data", kind="data", required=False, min_count=0),
            NodePort(id="graph", label="Graph", kind="graph", required=False, min_count=0),
            NodePort(id="evaluation", label="Evaluation", kind="evaluation", required=False, min_count=0),
            NodePort(id="evaluation_summary", label="Summary", kind="evaluation_summary", required=False, min_count=0),
            NodePort(id="graph_view", label="Graph View", kind="graph_view", required=False, min_count=0),
        ],
        output_ports=[NodePort(id="report", label="Report", kind="report", required=False)],
        preview=NodePreviewDefinition(enabled_by_default=True, supported_outputs=["report"]),
        inline_fields=["title"],
        fields=[
            NodeField(name="title", label="Title", kind="string", default="DAGBoard Experiment Report"),
        ],
    ),
]
